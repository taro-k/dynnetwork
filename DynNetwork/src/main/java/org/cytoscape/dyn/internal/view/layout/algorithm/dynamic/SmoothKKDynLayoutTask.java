 ...
 * for node layout. The node position is computed not only on the current time, but also based on
 * previous and past events, which guarantees smoothness of transition in time.
 * 
 ...
 * @param <T>
 */
public class SmoothKKDynLayoutTask<T> extends AbstractLayoutTask 
{
	...
	{	
		if (networkView!=null && dynView!=null)
		{
			...
			{
				snap.setInterval(new DynInterval<T>(events.get(Math.max(0,t-sfactor)),events.get(Math.min(events.size()-1,t+1+sfactor))));
